...
const RenderItems = ({arr, item, index}) => {
  return (
    <TouchableOpacity
      onPress={() => {
        NavService.navigate('Translationinfo', {item, arr, index});
      }}
      style={{
        marginBottom: 10,
        margin: 10,
...
class Translation extends PureComponent {
  ...
  getTranslationData = async () => {
    const data = await getApi('https://api.alquran.cloud/v1/quran/ur.asad');
  ...
  render() {
    ...
    return (
      <View style={{flex: 1}}>
        ...
        <View style={{flex: 8}}>
          <FlatList
            iinitialNumToRender={this.state.renderNumber}
            ListFooterComponent={() => {
            ...
            renderItem={({item, index}) => (
              <RenderItems
                arr={this.props.quranTranslationData}
                item={item}
                index={index}
              />
            )}
...
const mapStateToProps = ({reducer}) => {
  console.log('resssucer', reducer.quranTranslationData);
  return {
    quranTranslationData: reducer.quranTranslationData,
  };
};
