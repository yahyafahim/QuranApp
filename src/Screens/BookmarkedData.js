...
export class BookmarkedData extends Component {
  render() {
    const RenderItem = ({item, index}) => {
      onDelete = () => {
        let arr = this.props.bookMarkedData;
        arr.splice(index, 1);
        this.props.SavebookmarkedData([...this.props.bookMarkedData]);
      };
        ...
        <TouchableOpacity
          ...
          <View
            ...
            <Text style={{color: Colors.black, fontWeight: 600}}>
              Juz {item.ayahs[0].juz} - {item.ayahs[0].surah.englishName}
            </Text>
          </View>
    ...
    const arrayUniqueByKey = [
      ...new Map(
        this.props.bookMarkedData.map(item => {
...
export default connect(mapStateToProps, mapDispatchToProps)(BookmarkedData);
