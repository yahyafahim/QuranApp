...
const RenderItems = ({arr, item, index}) => {
  return (
    <TouchableOpacity
      onPress={() => {
        NavService.navigate('AudioInfo', {item, arr, index});
      }}
      style={{
        marginBottom: 10,
        margin: 10,
...
class AudioAyaat extends PureComponent {
  state = {
    data: [],
    surah: quranSurahs,
    isLoading: true,
  ...
  getData = async () => {
    const data = await getApi('https://api.alquran.cloud/v1/quran/ar.alafasy');
...
const mapStateToProps = ({reducer}) => {
  console.log('resssucer', reducer.quranData);
  return {
    quranAudioData: reducer.quranAudioData,
  };
};
export default connect(mapStateToProps)(AudioAyaat);
